...
class Hive_console:
    ...
    @_auto_reconnect
    def execute(self, command, verbose=False, to_df=False, _temp_file=False):
        '''
        From SQL running on hive, to Pandas DataFrame.
        Connect to Hive on Datalake, run the SQL query.
        First saved to a text file, then return a DataFrame.          
        ...
        _temp_file: whether keep temp text file, default False
            
        return: returns None if to_df is False, returns result Pandas DataFrame if set True.
        ...
        if (err_msg.lower().find('error') > 0) and (not verbose):
            # print out error message if necessary
            print(err_msg)
        temp_file = 'temp_file_' + str(time.time())[-6:]
        if to_df:
            # if to_df is True, method will return a Pandas DataFrame
            result_df = string_2_df(out_msg, temp_file)
            if not _temp_file:
                # delete the temp file generated
                os.remove(temp_file)
            return result_df
        else:
            print(out_msg)
